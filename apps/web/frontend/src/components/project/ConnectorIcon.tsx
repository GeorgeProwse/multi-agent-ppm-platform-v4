/**
 * ConnectorIcon - Resolves a connector icon name to the appropriate Icon component.
 */
...
const ICON_MAP: Record<string, IconSemantic> = {
...
export function ConnectorIcon({ name }: { name: string }) {
  const resolved = name.includes('.')
    ? (name as IconSemantic)
    : (ICON_MAP[name] ?? ICON_MAP.default);
