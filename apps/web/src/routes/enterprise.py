...
async def list_finance_budgets(workspace_id: str, request: Request) -> dict[str, Any]:
    ...
    return {"budgets": [b for b in store.get("budgets", []) if b.get("workspace_id") == workspace_id]}
...
async def export_finance_evidence(workspace_id: str, request: Request) -> dict[str, Any]:
    ...
    return {"workspace_id": workspace_id, "budgets": [b for b in budgets if b.get("workspace_id") == workspace_id], "change_requests": [c for c in changes if c.get("workspace_id") == workspace_id], "approvals": _approval_payload().get("approvals", [])}
...
async def sync_diff(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    ...
    for item in source:
        ...
        if not t:
            ...
            conflicts.append({"id": item["id"], "incoming": item, "current": t, "status": "requires_decision"})
...
async def compute_alerts(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    ...
    alerts = [{"id": "schedule-slip", "score": round((seed % 37) / 100 + 0.55, 3), "type": "schedule_slip_risk"}, {"id": "budget-overrun", "score": round((seed % 29) / 100 + 0.5, 3), "type": "budget_overrun_risk"}, {"id": "gate-readiness", "score": round((seed % 23) / 100 + 0.45, 3), "type": "gate_readiness_risk"}]
