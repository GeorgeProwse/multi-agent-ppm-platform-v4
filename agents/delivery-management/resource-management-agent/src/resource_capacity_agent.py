...
class ResourceCapacityAgent(BaseAgent):
    ...
    async def validate_input(self, input_data: dict[str, Any]) -> bool:
        ...
        if not action:
            ...
            return False
        valid_actions = {
            "add_resource", "update_resource", "delete_resource", "request_resource",
    ...
    async def process(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if action == "add_resource":
            return await handle_add_resource(self, input_data.get("resource", {}), tenant_id=tenant_id)
        if action == "update_resource":
            return await handle_update_resource(self, input_data.get("resource", {}), tenant_id=tenant_id)
        if action == "delete_resource":
            return await handle_delete_resource(self, input_data.get("resource_id", ""), tenant_id=tenant_id)
        if action == "request_resource":
            return await handle_request_resource(self, input_data.get("request", {}), tenant_id=tenant_id)
        if action == "approve_request":
            return await handle_approve_request(
                self, input_data.get("request_id", ""), input_data.get("approval_decision", {}), tenant_id=tenant_id
            )
        if action == "search_resources":
            return await handle_search_resources(self, input_data.get("search_criteria", {}))
        if action == "match_skills":
            return await handle_match_skills(self, input_data.get("skills_required", []), input_data.get("project_context", {}))
        ...
        if action == "allocate_resource":
            return await handle_allocate_resource(
                self, input_data.get("allocation", {}), tenant_id=tenant_id, correlation_id=correlation_id
            )
        if action == "get_availability":
            return await handle_get_availability(
                self, input_data.get("resource_id", ""), input_data.get("date_range", {}), tenant_id=tenant_id
            )
        ...
        *,
        ...
        if self.db_service:
            await self.db_service.store(
                ...
                {"resource_id": resource_id, "schedule": schedule, "availability": availability,
    ...
    async def _validate_resource_record(
        ...
        record = {
            "id": resource_profile.get("resource_id"), "tenant_id": tenant_id,
            "name": resource_profile.get("name"), "role": resource_profile.get("role"),
            "location": resource_profile.get("location"), "status": resource_profile.get("status"),
    ...
    async def _publish_resource_event(self, event_name: str, payload: dict[str, Any]) -> None:
        ...
        except (ConnectionError, TimeoutError, ValueError, KeyError, TypeError, RuntimeError, OSError) as exc:  # noqa: E501
    ...
    async def _deactivate_missing_resources(self, active_resource_ids: set[str]) -> None:
        ...
        for resource_id, resource in list(self.resource_pool.items()):
            if resource_id not in active_resource_ids and resource.get("source_system") in {
                "azure_ad", "workday", "sap_successfactors"
            }:
    ...
    async def _get_performance_score(self, resource_id: str, project_context: dict[str, Any]) -> float:
    ...
    def _risk_load_factor(self, risk_level: str | None) -> float:
        ...
        return float(self.risk_adjustments.get(level, self.risk_adjustments["default"]).get("resource_load_factor", 1.0))

    def _resolve_allocation_risk_level(self, allocation: dict[str, Any], risk_data: dict[str, Any] | None) -> str:
        if not risk_data:
            return "default"
    ...
    async def _check_availability(
        ...
        while current_date <= end:
            ...
            if day_avail.get("available_hours", 0) >= required_hours:
                windows.append({"start": current_date.isoformat(), "end": current_date.isoformat(), "capacity": effort})
            else:
                is_available = False
    ...
    async def _identify_capacity_bottlenecks(
        ...
        return [
            {"month": i, "capacity": float(cap.get("capacity", 0)),
    ...
    async def _generate_capacity_recommendations(self, bottlenecks: list[dict[str, Any]]) -> list[str]:
        if not bottlenecks:
            return []
        return [
    ...
    async def _generate_mitigation_strategies(self, gaps: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [
    ...
    async def _generate_hiring_recommendations(self, gaps: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [
            {"role": "Software Developer", "count": 2, "priority": "high"},
            {"role": "Project Manager", "count": 1, "priority": "medium"},
        ]

    async def _generate_training_recommendations(self, gaps: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [
            {"skill": "Cloud Architecture", "target_count": 3, "priority": "high"},
            {"skill": "Adaptive Methodologies", "target_count": 5, "priority": "medium"},
        ]

    async def _apply_scenario_changes(self, baseline: dict[str, Any], changes: dict[str, Any]) -> dict[str, Any]:
        added = changes.get("add_resources", [])
        removed = changes.get("remove_resources", [])
        scenario = dict(baseline)
        scenario["resource_count"] = max(0, baseline.get("resource_count", 0) + len(added) - len(removed))
    ...
    async def _calculate_scenario_metrics(self, scenario: dict[str, Any]) -> dict[str, Any]:
        ...
        avg_cap = sum(item["capacity"] for item in forecast["future_capacity"]) / max(len(forecast["future_capacity"]), 1)
        avg_dem = sum(item["demand"] for item in forecast["future_demand"]) / max(len(forecast["future_demand"]), 1)
    ...
    async def _compare_scenarios(self, baseline: dict[str, Any], scenario: dict[str, Any]) -> dict[str, Any]:
        return {
            "utilization_difference": scenario.get("average_utilization", 0) - baseline.get("average_utilization", 0),
            "resource_count_difference": scenario.get("resource_count", 0) - baseline.get("resource_count", 0),
            "cost_difference": scenario.get("cost_impact", 0) - baseline.get("cost_impact", 0),
            "schedule_difference": scenario.get("schedule_impact", 0) - baseline.get("schedule_impact", 0),
        }
    ...
    def get_capabilities(self) -> list[str]:
        ...
        return [
            "resource_pool_management", "demand_intake", "skill_matching",
