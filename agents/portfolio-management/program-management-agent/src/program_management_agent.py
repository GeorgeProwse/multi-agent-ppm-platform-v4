...
class ProgramManagementAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "program-management-agent", config: dict[str, Any] | None = None):
        ...
        self.dependency_types = (
            config.get("dependency_types", DEFAULT_DEPENDENCY_TYPES) if config else DEFAULT_DEPENDENCY_TYPES
        )
        self.synergy_detection_threshold = config.get("synergy_detection_threshold", 0.75) if config else 0.75
        self.health_score_weights = (
            config.get("health_score_weights", DEFAULT_HEALTH_SCORE_WEIGHTS) if config else DEFAULT_HEALTH_SCORE_WEIGHTS
        )
        self.optimization_objectives = (
            config.get("optimization_objectives", DEFAULT_OPTIMIZATION_OBJECTIVES)
            if config else DEFAULT_OPTIMIZATION_OBJECTIVES
        )

        self.program_store = TenantStateStore(
            Path(config.get("program_store_path", "data/program_store.json")) if config else Path("data/program_store.json")
        )
        self.roadmap_store = TenantStateStore(
            Path(config.get("program_roadmap_store_path", "data/program_roadmap_store.json"))
            if config else Path("data/program_roadmap_store.json")
        )
        self.dependency_store = TenantStateStore(
            Path(config.get("program_dependency_store_path", "data/program_dependency_store.json"))
            if config else Path("data/program_dependency_store.json")
        )
    ...
    async def process(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if action == "create_program":
            return await handle_create_program(self, input_data.get("program", {}), tenant_id=tenant_id, correlation_id=correlation_id)
    ...
    async def _identify_dependencies(
        self, project_ids: list[str], *, schedules: dict[str, Any] | None = None,
        ...
        for o in sched_overlaps:
            deps.append({"predecessor": o["project_a"], "successor": o["project_b"], "type": "schedule_overlap", "overlap_days": o["overlap_days"], "overlap_window": o["overlap_window"]})
        for o in res_overlaps:
            deps.append({"predecessor": o["project_a"], "successor": o["project_b"], "type": "shared_resource", "shared_resources": o["shared_resources"], "overlap_score": o["overlap_score"]})
        ...
        for pa, pb in combinations(project_ids, 2):
            ...
            if a0 <= b1 and b0 <= a1:
                deps.append({"predecessor": pa, "successor": pb, "type": "resource_contention", "shared_resources": shared, "overlap_window": {"start": max(a0, b0).date().isoformat(), "end": min(a1, b1).date().isoformat()}})
            elif a1 < b0:
                deps.append({"predecessor": pa, "successor": pb, "type": "resource_sequence", "lag_days": (b0 - a1).days, "shared_resources": shared})
            elif b1 < a0:
                deps.append({"predecessor": pb, "successor": pa, "type": "resource_sequence", "lag_days": (a0 - b1).days, "shared_resources": shared})

        for idx in range(len(project_ids) - 1):
            deps.append({"predecessor": project_ids[idx], "successor": project_ids[idx + 1], "type": self.dependency_types[idx % len(self.dependency_types)]})
        return deps

    async def _calculate_program_critical_path(self, schedules: dict[str, Any], dependencies: list[dict[str, Any]]) -> list[str]:
    ...
    async def _get_resource_allocations(self, project_ids: list[str]) -> dict[str, Any]:
        if self.resource_agent and project_ids:
            ...
            for pid in project_ids:
                r = await self.resource_agent.process({"action": "get_utilization", "project_id": pid})
    ...
    async def _get_project_benefits(self, project_ids: list[str]) -> dict[str, Any]:
        ...
        if self.business_case_agent and project_ids:
            ...
            for pid in project_ids:
                ...
                if bcid:
                    r = await self.business_case_agent.process({"action": "get_business_case", "business_case_id": bcid})
                    bd = r.get("benefit_breakdown", {}) or r.get("benefits", {})
                    benefits[pid] = {"total_benefits": bd.get("total_benefits", 0), "total_costs": r.get("total_cost", r.get("total_costs", 0)), "benefit_breakdown": bd}
            if benefits:
                return benefits
    ...
    async def _get_project_details(self, project_ids: list[str]) -> dict[str, Any]:
        if self.project_definition_agent and project_ids:
            ...
            for pid in project_ids:
                details[pid] = await self.project_definition_agent.process({"action": "get_charter", "project_id": pid})
            if details:
                return details
        ext = await ingest_external_program_data(self)
        if ext.get("projects"):
    ...
    async def _get_project_dependencies(self, program_id: str, project_id: str) -> list[dict[str, Any]]:
        stored = await self._get_dependency_graph(program_id)
        return [d for d in stored.get("dependencies", []) if project_id in d.values()]

    async def _estimate_project_costs(self, project_ids: list[str], *, tenant_id: str) -> dict[str, float]:
        costs: dict[str, float] = {}
        if self.financial_agent:
            for pid in project_ids:
                r = await self.financial_agent.process({"action": "get_financial_summary", "project_id": pid, "tenant_id": tenant_id, "context": {"tenant_id": tenant_id}})
                costs[pid] = float(r.get("total_cost") or r.get("total_costs") or r.get("budget_total", 0))
        if not costs:
            costs = {pid: 100000.0 for pid in project_ids}
        return costs

    async def _estimate_project_risks(self, project_ids: list[str], *, tenant_id: str) -> dict[str, float]:
        risks: dict[str, float] = {}
        if self.risk_agent:
            for pid in project_ids:
                r = await self.risk_agent.process({"action": "get_risk_dashboard", "project_id": pid, "tenant_id": tenant_id, "context": {"tenant_id": tenant_id}})
    ...
    # Event publishing
    async def _publish_program_status_update(self, program_id: str, *, tenant_id: str, correlation_id: str, status_type: str, payload: dict[str, Any]) -> None:
        sp = {"program_id": program_id, "status_type": status_type, "payload": payload, "updated_at": datetime.now(timezone.utc).isoformat()}
        if self.db_service:
            await self.db_service.store("program_status_updates", f"{program_id}-{status_type}-{uuid.uuid4().hex}", sp)
        await self.event_bus.publish("program.status.updated", {"program_id": program_id, "tenant_id": tenant_id, "status": sp, "correlation_id": correlation_id})

    # Cosmos dependency-graph CRUD
    async def _create_dependency_graph(self, program_id: str, dependencies: list[dict[str, Any]], *, tenant_id: str) -> dict[str, Any]:
        if not self.dependency_container:
            return {}
        item = {"id": program_id, "program_id": program_id, "tenant_id": tenant_id, "dependencies": dependencies, "updated_at": datetime.now(timezone.utc).isoformat()}
        await self.dependency_container.upsert_item(item)
        return item

    async def _update_dependency_graph(self, program_id: str, dependencies: list[dict[str, Any]], *, tenant_id: str) -> dict[str, Any]:
    ...
    async def _get_dependency_graph(self, program_id: str) -> dict[str, Any]:
        if not self.dependency_container:
            return {}
        try:
            return await self.dependency_container.read_item(item=program_id, partition_key=program_id)
    ...
    async def _compute_benefit_realization_metrics(self, program_id: str, project_ids: list[str]) -> dict[str, Any]:
        return await compute_benefit_realization_metrics(self, program_id, project_ids)

    async def _collect_external_health_signals(self, program_id: str, project_ids: list[str]) -> dict[str, Any]:
        return await collect_external_health_signals(self, program_id, project_ids)

    async def _generate_program_narrative(self, program: dict[str, Any], *, schedule_health: float, budget_health: float, risk_health: float, quality_health: float, resource_health: float, benefit_metrics: dict[str, Any]) -> str:
        return await generate_program_narrative(self, program, schedule_health=schedule_health, budget_health=budget_health, risk_health=risk_health, quality_health=quality_health, resource_health=resource_health, benefit_metrics=benefit_metrics)
