...
| `operations-management` | Agents for change management, release deployment, knowledge management, process mining, stakeholder communications, analytics, data quality, and system health. Contains the deprecated `workflow-engine-agent` directory (shared libraries consumed by the Approval Workflow agent). | [](/./operations-management/) |
