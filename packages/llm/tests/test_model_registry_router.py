...
def test_registry_filters_enabled_and_demo(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    registry_path.write_text(
        json.dumps(
            [
                {"provider": "openai", "model_id": "a", "display_name": "A", "enabled": True, "capabilities": ["chat"], "allow_in_demo": True},
                {"provider": "google", "model_id": "b", "display_name": "B", "enabled": False, "capabilities": ["chat"], "allow_in_demo": True},
                {"provider": "anthropic", "model_id": "c", "display_name": "C", "enabled": True, "capabilities": ["chat"], "allow_in_demo": False},
            ]
        ),
        encoding="utf-8",
    )
    ...
    assert [(item.provider, item.model_id) for item in enabled] == [("openai", "a"), ("anthropic", "c")]
...
async def test_router_rejects_unknown_model(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    registry_path.write_text(
        json.dumps(
            [
                {"provider": "openai", "model_id": "known", "display_name": "Known", "enabled": True, "capabilities": ["chat"], "allow_in_demo": True}
            ]
        ),
        encoding="utf-8",
    )
