"""MCP behavior tests for the Planview connector."""

from __future__ import annotations

import sys
from pathlib import Path
from typing import Any
from unittest.mock import patch

import pytest

REPO_ROOT = Path(__file__).resolve().parents[3]
CONNECTOR_SDK_PATH = REPO_ROOT / "connectors" / "sdk" / "src"
PLANVIEW_CONNECTOR_PATH = REPO_ROOT / "connectors" / "planview" / "src"
for path in (REPO_ROOT, CONNECTOR_SDK_PATH, PLANVIEW_CONNECTOR_PATH):
    if str(path) not in sys.path:
        sys.path.insert(0, str(path))

from base_connector import ConnectorCategory, ConnectorConfig, SyncDirection, SyncFrequency
from mcp_client import MCPServerError, MCPToolNotFoundError
from planview_connector import PlanviewConnector


class FakeMCPClient:
    def __init__(self, payload: Any, error: Exception | None = None) -> None:
        self.payload = payload
        self.error = error
        self.calls: list[dict[str, Any]] = []

    async def list_records(self, params: dict[str, Any]) -> Any:
        self.calls.append(params)
        if self.error:
            raise self.error
        return self.payload


@pytest.fixture
def planview_config() -> ConnectorConfig:
    return ConnectorConfig(
        connector_id="planview",
        name="Planview",
        category=ConnectorCategory.PPM,
        enabled=True,
        sync_direction=SyncDirection.INBOUND,
        sync_frequency=SyncFrequency.DAILY,
        instance_url="https://planview.example.com",
        prefer_mcp=True,
        mcp_server_url="https://mcp.example.com",
    )


def test_planview_mcp_invocation_and_mapping(planview_config: ConnectorConfig) -> None:
    payload = {"records": [{"id": "PV1", "title": "Roadmap", "status": "open"}]}
    fake_client = FakeMCPClient(payload)
    connector = PlanviewConnector(planview_config, mcp_client=fake_client)
    connector._authenticated = True

    with patch.object(connector, "_read_projects", return_value=[]):
        result = connector.read("projects", limit=10, offset=2)

    assert fake_client.calls == [
        {"resource_type": "projects", "filters": {}, "limit": 10, "offset": 2}
    ]
    assert result == [
        {
            "id": "PV1",
            "program_id": "unassigned",
            "name": "Roadmap",
            "status": "open",
            "start_date": None,
            "end_date": None,
            "owner": None,
            "classification": "internal",
            "created_at": None,
        }
    ]


@pytest.mark.parametrize(
    "error",
    [MCPToolNotFoundError("missing"), MCPServerError("rpc failed")],
)
def test_planview_mcp_fallbacks(planview_config: ConnectorConfig, error: Exception) -> None:
    fake_client = FakeMCPClient(payload={}, error=error)
    connector = PlanviewConnector(planview_config, mcp_client=fake_client)
    connector._authenticated = True
    rest_records = [{"id": "REST", "name": "Fallback"}]

    with patch.object(connector, "_read_projects", return_value=rest_records):
        result = connector.read("projects")

    assert result == [
        {
            "id": "REST",
            "program_id": "unassigned",
            "name": "Fallback",
            "status": "execution",
            "start_date": None,
            "end_date": None,
            "owner": None,
            "classification": "internal",
            "created_at": None,
        }
    ]
