...
def _load_router() -> types.ModuleType:
    module_name = "connectors.salesforce.src.router"
    ...
    main_stub = types.ModuleType("connectors.salesforce.src.main")
    main_stub.CONNECTOR_ROOT = Path(".")

    class _Config:
        @classmethod
        def from_env(cls, _token_url: str, _rate: int):
            return object()

    main_stub.SalesforceConfig = _Config
...
def test_sync_outbound_live_reports_partial_failures(monkeypatch) -> None:
    ...
    def _request(*_args, **kwargs):
        if kwargs["json"]["Id"] == "bad-1":
            raise router.HttpClientError("boom", status_code=429)
