...
class AsanaConnector(RestConnector):
    ...
    def _build_mcp_client(self) -> MCPClient:
        ...
        if not self.config.mcp_server_url:
            raise ValueError("Asana MCP server URL is required")
    ...
    def _try_mcp_then_rest(
        ...
        *,
        ...
        if not tool_name:
            logger.warning(
                "MCP tool mapping missing for %s; falling back to REST for Asana.",
                tool_key,
            )
            return rest_call()
        try:
            ...
            logger.warning(
                "MCP %s failed for Asana connector; falling back to REST. Error: %s",
                tool_key,
                exc,
            )
            return rest_call()
