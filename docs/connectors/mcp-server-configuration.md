...
Describe how to configure MCP servers for connector runtime routing, including authentication, scopes, and tool mapping.
...
- **Connector runtime fields**: the connector config model includes MCP-specific fields such as `mcp_server_url`, `mcp_server_id`, `mcp_client_id`, `mcp_client_secret`, `mcp_scope`, `mcp_api_key`, `mcp_api_key_header`, `mcp_oauth_token`, `mcp_tool_map`, and routing controls like `prefer_mcp`, `mcp_enabled_operations`, and `mcp_disabled_operations`. These fields drive MCP routing and auth header construction at runtime.
...
- Ensure the runtime configuration includes a `mcp_tool_map` entry; this is the authoritative mapping used by the MCP client when it calls `invokeTool`.
- Validate the tool names by calling `listTools` on the MCP server and confirming the tool names match the mapping keys.

## Enablement checklist

1. Add the MCP server URL, credentials, and scopes to the environment (or secret store) referenced in `config/README.md`.
...
3. Populate `mcp_tool_map` in the connector manifest and any project overrides so the router can resolve tool names.
