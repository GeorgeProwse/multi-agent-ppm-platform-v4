...
This guide provides internal onboarding steps, cross-system workflow examples, and troubleshooting tips for MCP-enabled connectors. It complements the MCP development guide and MCP server configuration docs by focusing on day-to-day enablement, operational checks, and project-level configuration.
...
2. **Validate tool inventory**
   - Call `listTools` on the MCP server and capture the tool names.
   - Build a canonical operation → tool mapping for the connector (this becomes `mcp_tool_map`).
...
4. **Create project-level overrides (optional but recommended)**
   ...
   - Override `mcp_tool_map`, `prefer_mcp`, or `mcp_enabled_operations` per project to control rollout scope.
...
- **Use canonical operations and tool maps:** Ensure the operation router has a complete `mcp_tool_map` before enabling MCP preference.
...
- **Document tool inventory:** Store `listTools` output in the connector’s internal notes so future updates can be validated quickly.
...
| --- | --- | --- |
| `Tool mapping missing for <operation>` | `mcp_tool_map` missing or incomplete | Update the project config or connector defaults with the missing operation key. |
...
| MCP calls falling back to REST | `prefer_mcp` disabled or missing tool map | Enable `prefer_mcp` and confirm tool names match `listTools`. |
...
| Empty `mcp_tools` list in UI | `mcp_tool_map` omitted in config | Populate `mcp_tool_map` so MCP tools are surfaced. |
